...
inputDialogues4 = LOAD 'hdfs:///user/root/episode4_d.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/root/episode5_d.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/root/episode6_d.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
